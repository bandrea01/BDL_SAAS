...
import { engCV, itaCV } from '../assets'
  ...
  return(
    <div className='xs:w-[250px] w-full justify-center' >
      <motion.div variants={fadeIn('right', 'spring', 0.5 *index, 0.75)} className='w-full green-pink-gradient p-[1px] rounded-[20px] shadow-card'>
        ...
        >
          <img src={icon}  draggable={false} alt={t(title)} className='object-contain w-16 h-16' />
          <h3 className='text-white text-[15px] mt-5 font-bold text-center'>{t(title)}</h3>
        </div>
      </motion.div>
    </div>
  )
...
const About = () => {
  ...
  return (
    ...
    <div className='items-center justify-center w-full mt-20 '>
      <motion.div variants={fadeIn('right', 'spring', 0.5 * services.length, 0.75)} className='w-full green-pink-gradient  p-[1px] rounded-[20px] shadow-card'>
        <a href={i18n.language == 'en' ? engCV : itaCV} download="CV_Andrenucci_Andrea">
          <div options={{
            max:45,
            scale:1,
            speed:450
