...
const LanguageDropdown = () => {
  ...
  const [flag, setFlag] = useState(
    sessionStorage.getItem("flag") || navigator.language.slice(0, 2) || "it"
  );
  const languages = [
  ...
  return (
    <div className="relative flex-row items-center inline-block">
      ...
      >
        <img
          src={flag == "it" ? itaFlag : engFlag}
          alt=""
          draggable={false}
          className="h-6 rounded w-9"
        />
        <div
          className={` transform transition-transform duration-400 ease-in-out w-0 h-0 border-l-[8px] border-l-transparent border-t-[7px] border-r-[8px] border-r-transparent ${
            isRotated ? "rotate-180" : ""
          } ${isScrolled ? "border-t-white" : "border-t-blue"}`}
        />
      </div>
      {isOpen && (
